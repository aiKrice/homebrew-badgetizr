...
EOF
    ...
    if [ -f "$MOCK_GH_RESPONSES_DIR/labels_db.txt" ]; then
        grep "work in progress" "$MOCK_GH_RESPONSES_DIR/labels_db.txt" | grep -q "Generated by Badgetizr - do not edit"
    fi
}
...
@test "WIP label removed when title changes (integration)" {
    ...
    wip_labelized_config=$(create_temp_config "$(cat <<EOF
badge_wip:
  enabled: "true"
  settings:
    color: "yellow"
...
@test "Hotfix: Badgetizr-managed labels are removed correctly" {
    ...
    mkdir -p "$MOCK_GH_RESPONSES_DIR"
    echo "hotfix|d73a49|Generated by Badgetizr - do not edit" > "$MOCK_GH_RESPONSES_DIR/labels_db.txt"
...
EOF
    ...
    if [ -f "$MOCK_GLAB_RESPONSES_DIR/labels_db.txt" ]; then
        grep "work in progress" "$MOCK_GLAB_RESPONSES_DIR/labels_db.txt" | grep -q "Generated by Badgetizr - do not edit"
    fi
}
